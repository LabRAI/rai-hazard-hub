<script lang="ts">
  ...
  let topLocations: PageViewLocation[] = $derived(
    locationsWithPosition.slice(0, 2),
  );
  $effect(() => {
    if (mapReady) {
    ...
    <div class="visitor-map-card">
      <div class="visitor-map-card-header">
        <div>
          ...
          <strong
            >{loading ? "..." : formatCount(summary.totalViews)} views</strong
          >
        </div>
        <div class="visitor-total-pill">IP hidden</div>
      </div>
      ...
      {:else if !loading}
        <div class="visitor-empty-state">
          Traffic markers appear after public visits.
        </div>
      {/if}
